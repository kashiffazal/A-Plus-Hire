...
const NDISProviderForm = (pr) => {
  ...
  const [companyLogoOnLoad, setCompanyLogoOnLoad] = useState('')
  ...
  const Submit = (values) => {
    ...
    setTimeout(() => {
      if (stepsCount === maxStepCount) {
        ...
        setLoader(true)
        HTTP('post', '/ndisForm', values).then(res => {
          setLoader(false);
          if (!res) { return false; }
          setShowSuccess(true);
          NextStep();
          setProgressStatus(100);
          //# Update cookies with 'complete_by_type' as true, NDIS id and profile Image on complete
          cookieState.complete_by_type = true;
          cookieState.ndis_ref_id = res.id;
  ...
  const GetData = () => {
    setGetLoader(true);
    HTTP('get', '/ndisFormData/' + (pr.id ? pr.id : '')).then(res => {
      setGetLoader(false);
      if (!res) { return false; }
      // console.log(res);
      ...
      if (pr.id) {
        ...
        if (res.data.company_logo) { setCompanyLogoOnLoad(res.data.company_logo); delete res.data.company_logo; }//End if condition
  ...
  const formName = 'NDIS Provider Form';
  return (
            ...
            !showSuccess ?
              <div className="bg-white p-4 sm:p-6 lg:px-8 lg:py-7 rounded-b">
                <Form onFinish={Submit} layout="vertical" className='form-style form-text text-base' autoComplete="off" ref={formRef}>
                  ...
                  {stepsCount === 1 && <STEP_1 onChangeField={OnChangeField} fv={formValues} headTitle={headTitle} context={context} fp={fp} userImgNew={userImgNew} setUserImgNew={setUserImgNew} userImgOld={userImgOld} companyLogoOnLoad={companyLogoOnLoad} />}
...
export default NDISProviderForm;