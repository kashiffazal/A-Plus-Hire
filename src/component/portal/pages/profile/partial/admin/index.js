...
import { HTTP, RandomAlphaNumber, Logout, UpdateUserCookie, SetNumberMask } from '../../../../../services';
...
const AdminProfile = (pr) => {
  ...
  const SubmitForm = (values) => {
    ...
    HTTP('post', '/updateUserProfile', { ...values, profile_image: profileImageNew }).then(res => {
      ...
      let updatedData = { ...pr.context.data.ud, ...res.data };
      pr.context.updateData({ ud: updatedData });
      pr.updateData && pr.updateData(updatedData);
    });
  }//End function
  ...
  const ToggleForm = () => {
    ...
    setTimeout(() => { SetNumberMask('mobile_number') }, 500);
  }//End function
                ...
                <div className='md:flex w-full [&>*]:mb-5 [&>*]:md:mb-0'>
                  <div className='lg:w-[23.3%] md:mr-10 lg:mr-[70px] xl:mr-10 text-center md:text-left'>
                    {toggleEdit ?
                      ...
                      :
                      <div className='border-[3px] border-solid border-[var(--colorPrimary)] rounded-[5px] p-[4px] !mb-[-1px]'>
                  ...
                  <div className='w-full md:w-[38.3%] md:mr-5 text-center md:text-left'>
                    ...
                    {toggleEdit ?
                      <div className='mt-[11px] [&>*]:!mb-[7px] [&>:last-child]:!mb-0'>
                        ...
                        <AntInput size="large" className="h-[48px]" type="email" name="email" placeholder="Email Address" />
                        <AntInput size="large" className="h-[48px]" name="mobile_number" placeholder="Mobile Number (Optional)" noRequired={true} rules={{ min: process.env.REACT_APP_MOBILE_FORMAT_NUMBER.length - 2, message: 'Invalid Number' }} />
                      </div>
                      :
                      <List
                        ...
                        dataSource={[
                          ...
                          { title: 'Email Address', description: ud.email },
                          { title: 'Mobile Number', description: ud.mobile_number }
                        ]}
                      ...
                      <>
                        <List
                          ...
                          dataSource={[
                            ...
                            { title: 'Updated At', description: ud.updatedDate + ', ' + ud.updatedTime }
                          ]}
                          className='hidden md:block [&>.ant-spin-nested-loading>.ant-spin-container>.ant-list-items>.ant-list-item:last-child]:!pb-0'
                          renderItem={(item, index) => (
                            <List.Item className='!py-[4px]'><List.Item.Meta title={<span className='font-medium'>{item.title}</span>} description={<span className='text-gray-900'>{item.description}</span>} /></List.Item>
                          )}
                        />
                        <Button size="large" type="primary" htmlType="submit" className='w-full md:mt-[9px] !h-[48px]' loading={btnLoader}>Update Profile</Button>
                      </>
                      :
                      <List
                        itemLayout="horizontal"
                        dataSource={[
                          { title: 'Status', description: <span className='success-link font-semibold'>{ud.status}</span> },
                          { title: 'Created At', description: ud.createdDate + ', ' + ud.createdTime },
                          { title: 'Updated At', description: ud.updatedDate + ', ' + ud.updatedTime }
                        ]}
                        className='[&>.ant-spin-nested-loading>.ant-spin-container>.ant-list-items>.ant-list-item:last-child]:!pb-0'
