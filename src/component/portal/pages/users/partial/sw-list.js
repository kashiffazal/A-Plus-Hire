...
const SWList = () => {
  ...
  const UpdateStatus = (row, status) => {
    ...
    HTTP('get', '/swUpdateStatus/' + row.id + '/' + status).then(res => {
        ...
        <div className="menu-icon-container">
          {row.complete_by_type ?
            <Tooltip placement="topRight" title='View Details' mouseEnterDelay={0.5}>
              <button onClick={() => { setRoleType(row.type); setEditAndViewId(row.sw_ref_id); setOpenViewModal(true); }}><i className="las la-table" /></button>
            </Tooltip>
            :
            <Popover placement="topLeft" content={`This user did not complete their ${row.role} profile.`} title="Can not view data">
              <button><i className="las la-comment-slash pending-link" /></button>
            </Popover>
          }
          <i className="las la-redo" />
          {row.complete_by_type ?
            <Tooltip placement="top" title='Edit' mouseEnterDelay={0.5}>
              <button onClick={() => { setRoleType(row.type); setEditAndViewId(row.sw_ref_id); setOpenForm(true); }}><i className="las la-edit" /></button>
            </Tooltip>
            :
  ...
  return (
    <ScreenLoader active={getLoader}>
      <DataTable
        ...
        label=<span className='font-semibold'>Support Worker <span className='text-[var(--colorPrimary)]'>Log</span></span>
        desc="Support worker with view, edit and change status functionality"
        customFilterCol={[
