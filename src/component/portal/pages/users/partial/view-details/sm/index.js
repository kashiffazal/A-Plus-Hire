...
const SMProfile = (pr) => {
  ...
  const GetData = (id) => {
    setLoader(true);
    HTTP('get', '/smViewDetails/' + id).then(res => {
      setLoader(false);
      if (!res) { return false; }
      setData(res.data);
    });
      ...
      <ScreenLoader active={loader}>
        ...
        <div className='mt-5 grid md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-2 md:gap-5'>
          <div className='lg:col-span-2 xl:col-span-2 portal-container !rounded-xl'>
            ...
            <div className='pt-1 mb-1 whitespace-pre-wrap h-[207px]'>
              {data.about_you}
            </div>
          </div>
          ...
          <div className='portal-container !rounded-xl'>
            ...
            <h2 className='mt-4 text-[18px] font-semibold'>Services Need</h2>
            <hr className='border-dashed border-gray-300 my-[3px]' />
            <List
              itemLayout="horizontal"
              dataSource={[
                {
                  title: 'What NDIS support services you are need help with?', description: <span>
                    {data.services_you_need && data.services_you_need.map(item => {
