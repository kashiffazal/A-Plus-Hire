...
const NDISList = () => {
  ...
  const UpdateStatus = (row, status) => {
    ...
    HTTP('get', '/ndisUpdateStatus/' + row.id + '/' + status).then(res => {
  ...
  const columns = [{
    ...
    ...TableColumnFilter(tableIndividualColFilter, 'company_name', filterIndividualColArr),
    render: (record, row) => <div>{record}</div>
  }, {
    title: 'Email',
    dataIndex: 'email',
      ...
      <ScreenLoader active={updateStatusLoader[row.id]} inline={true} tip="Loading...">
        <div className="menu-icon-container">
          <Tooltip placement="topRight" title='View Details' mouseEnterDelay={0.5}>
            <button onClick={() => { setRoleType(row.type); setEditAndViewId(row.ndis_ref_id); setOpenViewModal(true); }}><i className="las la-table" /></button>
          </Tooltip>
          <i className="las la-redo" />
          {row.complete_by_type ?
            <Tooltip placement="top" title='Edit' mouseEnterDelay={0.5}>
              <button onClick={() => { setRoleType(row.type); setEditAndViewId(row.ndis_ref_id); setOpenForm(true); }}><i className="las la-edit" /></button>
            </Tooltip>
            :
  ...
  return (
    <ScreenLoader active={getLoader}>
      <DataTable
        ...
        label=<span className='font-semibold'>NDIS Provider <span className='text-[var(--colorPrimary)]'>Log</span></span>
        desc="NDIS Provider with view, edit and change status functionality"
        customFilterCol={[
