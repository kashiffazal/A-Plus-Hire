...
const { SetUserTypeAsName, Base64ToImage, StringToArrForDB, ArrToStringForDB } = require('../../services');
const dayjs = require('dayjs');

module.exports = {

  SMFormData: async (req, res) => {
    let resp = { status: true };
    //@ If id is available then get data from DB ===============//
    if (req.params.id) {
      resp = await DBServices.GetData(db.self_managed_participant, {
      ...
      if (resp.data['services_you_need']) { resp.data['services_you_need'] = StringToArrForDB(resp.data['services_you_need'], true); };
    }//End if condition
    //@ ========================================================// 
  ...
  SMFormPost: async (req, res) => {
    let dt = req.body;
    //@ Convert Array into comma separated string
    let arrToString = ['services_you_need'];
    ...
    //# Post Data
    let response = await DBServices.PostOrUpdate(db.self_managed_participant, dt);
    //# Some steps after insert or update
    if (response.status) {
      //# Update status for complete form in user table
      await DBServices.UpdateDataById(db.users, { 'id': dt.user_ref_id, sm_ref_id: response.id, complete_by_type: true });
      //# Save image into folder if available
      if (profile_image && response.id) {
        var fileName = await Base64ToImage(profile_image, dt.user_ref_id + '-sm-img.png', process.env.PROFILE_IMG_PATH);
        if (fileName.status) {
  ...
  SMList: async (req, res) => {
    let response = await DBServices.GetDataList(db.users, {
      attributes: ['id', 'first_name', 'last_name', 'email', 'mobile_number', 'profile_image', 'suburb', 'state', 'type', 'sm_ref_id', 'status', 'complete_by_type'],
      where: { type: 'sm' },
      include: [
  ...
  SMViewDetails: async (req, res) => {
    let response = await DBServices.GetData(db.self_managed_participant, {
      where: { id: req.params.id },
      include: [{
    ...
    if (response.status) {
      ...
      if (response.data.services_you_need) {
        let dropdown = await DBServices.GetDataList(db.list_dropdown, {
          attributes: ['id', 'name'],
          where: { id: StringToArrForDB(response.data.services_you_need) }
        });
        response.data.services_you_need = dropdown.data.reverse();
      }//End if condition
      ...
      response.data.userData.package_name = ud.packageData.name ? ud.packageData.name : '';
      //@ Setting dates
